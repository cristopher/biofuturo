export class make{
    ...
    static modal(button){
        ...
        let resultado ={
            ...
            contenido:contenido,
            button:_button,
            modal:'<div class="modal fade" tabindex="-1" role="dialog" id="'+id+'"><div class="modal-dialog modal-lg modal-dialog-scrollable" role="document"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="'+titulo+'">Modal title</h5></div><div class="modal-body text-body" id="'+contenido+'"></div><div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancelar</button>'+ button_string+'</div></div></div></div>'
        }
    ...
    static alert(mensaje, alerta = false){
        ...
        document.getElementsByClassName("modal-footer")[0].children[0].textContent = "Aceptar"
        //the(modal.id + " button").html("Aceptar");
        ...
        const myModalEl = document.getElementById(modal.id)
        myModalEl.addEventListener('hidden.bs.modal', event => {
            the(event.currentTarget.id).remove()
        })
    }
